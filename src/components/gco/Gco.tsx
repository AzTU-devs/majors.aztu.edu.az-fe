    ...
    <SpecialtyShell specialtyCode={specialtyCode} active="graduate-career-opportunities" subtitle={subtitle}>
      {loading ? (
        <div className="grid gap-4 md:grid-cols-2">
          {[0,1,2,3].map(i => <Skeleton key={i} variant="rectangular" height={140} className="rounded-2xl" />)}
        </div>
      ) : items.length === 0 ? (
          ...
          {items.map((g, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#182f79]/10 text-[#182f79]">
