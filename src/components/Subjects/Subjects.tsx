    ...
    <SpecialtyShell specialtyCode={specialtyCode} active="subjects" subtitle={subtitle}>
      {loading ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[0,1,2,3,4,5].map(i => <Skeleton key={i} variant="rectangular" height={160} className="rounded-2xl" />)}
        </div>
      ) : items.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {items.map((sub) => (
            <Link
              ...
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
              <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#182f79] to-[#3b4fc2]" />
              <div className="flex items-center justify-between">
                <span className="rounded-lg bg-gray-100 px-2.5 py-1 font-mono text-[11px] font-medium text-gray-600">
                  {sub.subject_code}
                </span>
                <span className="rounded-full bg-[#182f79]/10 px-2.5 py-0.5 text-[11px] font-semibold text-[#182f79]">
              ...
              <dl className="mt-4 grid grid-cols-3 gap-2 text-[11px] text-gray-500">
                <div>
                  <dt className="font-medium uppercase tracking-wide">{locale === "az" ? "Kredit" : "Credits"}</dt>
                  <dd className="mt-0.5 text-sm font-semibold text-gray-900">{sub.credit}</dd>
                </div>
                <div>
                  <dt className="font-medium uppercase tracking-wide">{locale === "az" ? "Saat/həftə" : "Hrs/wk"}</dt>
                  <dd className="mt-0.5 text-sm font-semibold text-gray-900">{sub.hours_per_week}</dd>
                </div>
                <div>
              ...
              <span className="mt-5 inline-flex items-center gap-1 text-sm font-medium text-[#182f79] transition-all group-hover:gap-2">
